...
import { answerQuestionsPrompt } from './prompts';
...
type Message = {url: string, user: string, password: string};
...
const allowedDomains = [
  ...
  { name: 'ag3nts', url: 'xyz.ag3nts.org', scrappable: true },
];
...
app.post('/api/hack', async (req, res) => {
  ...
  try {
    ...
    const items = await webSearchService.scrapeUrls([req.body.url]);
    console.dir(items);
    const answer = await openaiService.completion([{
      ...
      <Context>
      USER: ${ items[0].content}
      </Context>
      `
     
    }], "gpt-4o-mini", false);

    console.dir(answer.choices[0].message.content);
    try{
      // The data you want to send to the form
      const formData = {
      ...
      return res.status(200).contentType('text/html').send(response.data);
    } catch (error) {
    ...
    // const shouldSearch = await webSearchService.isWebSearchNeeded(message.url as string);
